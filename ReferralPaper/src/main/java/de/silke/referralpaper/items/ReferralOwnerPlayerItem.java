...
public class ReferralOwnerPlayerItem {
    ...
    public static GuiItem getHead(UUID playerUUID) {
        ...
        // Ник игрока
        String playerName = Main.plugin.getReferralsDatabaseConnection().getPlayerName(playerUUID).join();
        // Роль игрока
        ...
        int invites = Main.plugin.getReferralsDatabaseConnection().getAmountOfTimesUsed(playerUUID).join();
        // Ник пригласившего игрока
        String invitedBy = Main.plugin.getPlayerInfoDatabaseConnection().getReferredPlayerName(playerUUID).join();
        ...
        // Количество приглашённых игроков
        if (invites != 0) {
            lore.add(ChatColor.GRAY + "Пригласил: " + ChatColor.WHITE + invites + " игроков");
        } else {
